	...
	)
{
    ...
    float* accum_sem = smem + (tid * 3) * num_channels;          // [num_channels]
    float* dL_feat    = smem + (tid * 3 + 1) * num_channels;     // [num_channels]
    float* last_sem   = smem + (tid * 3 + 2) * num_channels;     // [num_channels]
	for (int ch = 0; ch < num_channels; ++ch) {
        accum_sem[ch] = 0.0f;
        dL_feat[ch]   = inside ? dL_dfeaturepixels[ch * H * W + pix_id] : 0.0f;
        last_sem[ch]  = 0.0f;
    }
	...
	//float accum_semantic_feature_rec[num_channels] = { 0 }; 
	float dL_dpixel[C];
	float dL_dpixel_obj[O];
	//float dL_dfeaturepixel[num_channels];
	...
	//float last_semantic_feature[num_channels] = { 0 };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const int base = global_id * num_channels;
			for (int ch = 0; ch < num_channels; ch++) 
			{
				const float f = semantic_feature[base + ch];
				accum_sem[ch] = last_alpha * last_sem[ch] + (1.f - last_alpha) * accum_sem[ch];
				last_sem[ch] = f;

				const float g = dL_feat[ch];
				dL_dalpha += (f - accum_sem[ch]) * g;
				atomicAdd(&(dL_dsemantic_feature[base + ch]), dchannel_dfeature * g);
			}	
			dL_dalpha *= T;
...
{
	renderCUDA<NUM_CHANNELS, NUM_OBJECTS> << <grid, block, shmem_bytes>> >(
		ranges,
		point_list,
		W, H,
		num_channels,
