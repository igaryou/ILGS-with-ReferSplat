	...
	float* __restrict__ out_feature_map)
{
	...
	//float SF[num_channels] = { 0 };
	extern __shared__ float smem[];
    // 1スレッドあたり num_channels 要る。タイル内 BLOCK_SIZE スレッド分を確保。
    // 共有メモリの先頭にこのピクセル(=このスレッド)の書き込み領域を割り当てる
    int tid = threadIdx.y * BLOCK_X + threadIdx.x;
    float* SF = smem + tid * num_channels;

    // 初期化
    for (int ch = 0; ch < num_channels; ++ch) SF[ch] = 0.0f;  
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < num_channels; ch++){
				SF[ch] += semantics[collected_id[j] * num_channels + ch] * alpha * T;}
			T = test_T;
